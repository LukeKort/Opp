...
from parameters import pso_only
...
x=radom_generator(n_variables,n_particles,a,b) #position matrix
v=radom_generator(n_variables,n_particles,a,b) #velocity matrix
x_best = x[:,rand.randrange(0,(n_particles-1))]
x_aux=x; #stores the best value's position per particle

t_0 = time.time() #start time

for i in range(ext_cicles):  
    ...
    for j in range(n_particles):
        ...
        x[:,j]=x[:,j]+v[:,j] #new position matrix
        if (constraints(x[:,j])) is True: #teste the new x within the constraints functions
            if (objective(x[:,j])) < objective(x_0[:,j]): #teste the new x within the objetive function
    ...
    # if tolerance >= np.amax(abs(x-x_aux)): #break for setting tolerance
    #     break
